...
export function airDensity(elevation=0,temperatureC=15){const T=273.15+temperatureC;const P=101325*Math.pow(1-0.0000225577*elevation,5.25588);return P/(287.05*T)}
export function siteAirDensity(){const p=S?.project;if(!p)return 1.225;if(S?.era5?.avgAirDensity)return S.era5.avgAirDensity;const elev=p.elevation||p.siteElevation||0;const temp=p.temperature||p.siteTemperature||15;const rho=airDensity(elev,temp);return rho}
