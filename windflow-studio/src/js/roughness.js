...
export async function downloadRoughness(){let T=S.terrain;if(!T){const pts=S.turbines?.length?S.turbines:[S.project];const lats=pts.map(p=>p.lat),lons=pts.map(p=>p.lon);const pad=.05;T={lat0:Math.min(...lats)-pad,lat1:Math.max(...lats)+pad,lon0:Math.min(...lons)-pad,lon1:Math.max(...lons)+pad};log('Roughness: using layout/site bounding box because terrain is not loaded','w')}const q=`[out:json][timeout:25][bbox:${T.lat0},${T.lon0},${T.lat1},${T.lon1}];(way["landuse"];way["natural"~"wood|water|scrub|grassland|wetland|sand|bare_rock"];relation["landuse"]["type"="multipolygon"];);out geom;`;try{const r=await fetch('https://overpass-api.de/api/interpreter',{method:'POST',body:'data='+encodeURIComponent(q)});if(!r.ok)throw Error('Overpass HTTP '+r.status);const d=await r.json();const map={farmland:.03,grass:.03,meadow:.03,grassland:.03,scrub:.05,forest:1.5,wood:1.5,residential:.4,commercial:.4,industrial:.5,water:.0002,wetland:.0002,retail:.3,construction:.1,orchard:.1,sand:.003,desert:.003,bare_rock:.01};S.roughness=(d.elements||[]).filter(e=>e.geometry&&e.geometry.length>2).map(e=>{const lu=e.tags?.landuse||e.tags?.natural||'other';return{lu,z0:map[lu]||.03,pts:e.geometry.map(p=>({lat:p.lat,lon:p.lon}))}});log(`OSM roughness: ${S.roughness.length} polygons`);return S.roughness}catch(e){S.roughness=[];log(`Roughness fallback: using project z0=${S.project.z0} (${e.message})`,'w');return S.roughness}}
