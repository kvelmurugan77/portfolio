import {S,log} from './state.js';import {$} from './utils.js';import {generateGrid,parseLayoutFile} from './layout.js';import {downloadTerrain} from './terrain.js';import {downloadRoughness} from './roughness.js';import {downloadGWA,setGWAFromText,setGWAFromTimeSeries} from './gwa.js';import {downloadERA5} from './era5.js';import {runAEP} from './flow.js';import {runTimeSeries} from './timeseries.js';import {parseMastClimate} from './mastClimate.js';import {parseMastTimeSeries} from './mastTimeSeries.js';import {parseObstacles} from './shelter.js';import {parsePowerCtCurve} from './powerCurve.js';import {exportReport,exportTsCsv} from './report.js';import {refresh,readProject,setupTabs} from './ui.js';
...
$('btnLoadLayout').onclick=async()=>{try{const f=$('layoutFile').files[0];if(!f)return alert('Choose CSV/TXT/KML/KMZ');await parseLayoutFile(f);refresh()}catch(e){alert('Layout import failed: '+e.message)}};
...
$('btnGWA').onclick=async()=>{try{readProject();await downloadGWA(S.project.lat,S.project.lon,S.project.hubHeight,S.project.z0);refresh()}catch(e){if(S.era5?.sp?.length){setGWAFromTimeSeries(S.era5.sp,S.era5.dir,S.project.hubHeight,S.project.z0,S.era5.height||100);status('GWA blocked; using ERA5-derived fallback climate. For exact GWA, import .lib file.');refresh()}else status('GWA blocked by browser/CORS. Download ERA5 first for fallback, or import GWA .lib.')}};
